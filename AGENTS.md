...
10. 每套项目只生成一个讲解PPT，不得额外生成独立口播稿、技术报告、事实校验表或操作说明。PPT内部包含主讲页面、逐页演讲者备注和固定品牌结束页。
...
14. 每张候选页面必须通过页面独立价值门槛；简单技术主讲通常控制在7至9页，无独立价值页面必须合并、删除或转入备注。
15. PPT主体默认先讲核心技术、真实缘由和可执行规则，再用案例证明观众已经理解；简单定义、缘由和规则允许合并。
16. 主讲页优先使用准确自然的人话；公式、程序变量和正式口径进入备注或GitHub内部记录。自然语言不得改变方向、循环、边界或冻结规则。
...
22. GitHub内部证据必须保存本金、止盈和止损的计算依据、模式、金额、比例、达到后动作、软件证据和替代停止条件；主讲与内部证据金额必须完全一致。
23. 证据必须保存足够的样本、命中、基准、统计、版本、验证状态和执行边界；不得依赖隐藏附录掩盖关键事实。
...
30. 自动返工最多3轮。超过上限必须进入BLOCKED并保留失败证据，不得伪造通过。
31. 所有变更先归类到PPT、SCHEME、PROGRAM、SYSTEM或CLEANUP扩展域；跨域变更必须提交影响报告并运行各域专项校验。
32. 清理任务默认只生成计划，必须完成引用扫描、先隔离后删除、恢复清单和清理前后校验；不得在普通功能PR中顺手批量删除。
