...
60. 每个正式候选必须完成连续挂10、20、30、40、50期的累计投入、剩余本金、下一期投入、命中净结果、回收状态和生存状态计算。
61. 任何“一期回本”“连本带利”结论必须有明确赔率与成本公式；结果为负时不得标记完成回收。
62. 每个标准方案必须完成理论概率、确定性连挂压力、至少10000条随机路径压力模拟和历史验证四层证据。
63. 历史数据少于1000期时只允许标记快速实验；少于10000期不得声称长期安全、稳定盈利或不容易亏损。
64. 历史数据达到1000期必须分训练和验证；达到5000期必须滚动验证；达到10000期必须保留冻结前不可查看的封存样本外区。
65. 正式入选资金路径必须有E3及以上软件执行证据，并在声明停止期仍能支付下一期；否则退回导演阶段。
