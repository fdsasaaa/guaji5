...
def add_money_badge(slide) -> None:
    # Vector embellishment, no external binary dependency.
    cx, cy = Inches(11.65), Inches(0.86)
    bag = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.OVAL, cx, cy, Inches(1.00), Inches(1.00))
    bag.fill.solid(); bag.fill.fore_color.rgb = RGBColor(229, 173, 49)
    bag.line.color.rgb = RGBColor(255, 219, 115); bag.line.width = Pt(2)
    add_text(slide, cx, cy + Inches(0.03), Inches(1.00), Inches(0.94), "$", 34, (38, 40, 39), True, PP_ALIGN.CENTER)
    for dx, dy, sz in [(0.82, 0.73, 0.20), (0.95, 0.62, 0.14), (-0.08, 0.78, 0.16)]:
        c = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.OVAL, cx + Inches(dx), cy + Inches(dy), Inches(sz), Inches(sz))
        c.fill.solid(); c.fill.fore_color.rgb = RGBColor(240, 190, 62)
        c.line.color.rgb = RGBColor(255, 223, 126)


def build_second_slide(prs: Presentation):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    bg = slide.background.fill
    bg.solid(); bg.fore_color.rgb = RGBColor(4, 7, 10)
...
def main() -> None:
    ...
    # Convert to PNG to keep a predictable Office background image format.
    ...
    cover_prs = new_prs(); add_cover(cover_prs, cover_png)
    save_presentation(cover_prs, ASSET_DIR / "固定首页模板.pptx")

    second_prs = new_prs(); build_second_slide(second_prs)
    save_presentation(second_prs, ASSET_DIR / "固定第二页_平台与联系.pptx")

    end_prs = new_prs(); add_end(end_prs, end_png)
    save_presentation(end_prs, ASSET_DIR / "固定最后一页_标准版.pptx")

    full = new_prs(); add_cover(full, cover_png); build_second_slide(full); add_end(full, end_png)
    save_presentation(full, ASSET_DIR / "PPT固定首页第二页末页模板_V3.9.3.pptx")

    files = [
        cover_jpg, end_jpg, cover_png, end_png,
