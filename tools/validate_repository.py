...
def load_jsonl(path,key):
 ...
 return out
required=['AGENTS.md','SYSTEM_MANIFEST.json','SYSTEM_STATE.json','系统状态.json','当前任务.json','CHANGELOG.md','00A_当前强制覆盖与废止规则.md','05A_方案讲解PPT生产协议.md','05B_固定首页与末页协议.md','11A_本金止盈止损设计与PPT披露协议.md','PPT页面类型卡片.jsonl','PPT讲解验收测试集.jsonl','PPT压缩与精度规则.json','10_静默方案总控与外部参考吸收协议.md','11_智能功能调度与资金路径编排协议.md','13_GitHub持续工作区与参考灵感自由重构协议.md','功能能力卡片.jsonl','资金路径模板库.jsonl','.github/workflows/validate.yml']
for x in required:
 if not (ROOT/x).exists(): err(f'缺少必需文件: {x}')
if (ROOT/'05B_固定首页第二页末页协议.md').exists(): err('仍保留已废止的固定第二页协议文件')
m=load_json(ROOT/'SYSTEM_MANIFEST.json'); s=load_json(ROOT/'SYSTEM_STATE.json'); sc=load_json(ROOT/'系统状态.json'); task=load_json(ROOT/'当前任务.json'); precision=load_json(ROOT/'PPT压缩与精度规则.json')
version=m.get('版本')
...
jsonl_specs={'历史方案索引.jsonl':'方案ID','分析角度索引.jsonl':'角度ID','学习事件索引.jsonl':'事件ID','规则候选池.jsonl':'候选规则ID','技术原子表现档案.jsonl':'技术原子ID','软件行为证据索引.jsonl':'证据ID','批次索引.jsonl':'批次ID','方案组合案例索引.jsonl':'案例ID','负面方案模式索引.jsonl':'负面模式ID','功能覆盖索引.jsonl':'功能ID','总控验收测试集.jsonl':'测试ID','功能能力卡片.jsonl':'功能ID','资金路径模板库.jsonl':'资金路径ID','PPT页面类型卡片.jsonl':'页面类型ID','PPT讲解验收测试集.jsonl':'测试ID'}
loaded={n:load_jsonl(ROOT/n,k) for n,k in jsonl_specs.items()}
pt=loaded['PPT页面类型卡片.jsonl']; ids={x.get('页面类型ID') for x in pt}; required_ids={'COVER','TECH_DEFINE','TECH_REASON','RULE','STEP','CASE','DATA','COMPARE','RISK','ADVICE','CONCLUSION','BRAND_END'}
...
override=(ROOT/'00A_当前强制覆盖与废止规则.md').read_text(encoding='utf-8'); proto=(ROOT/'05A_方案讲解PPT生产协议.md').read_text(encoding='utf-8'); fixed=(ROOT/'05B_固定首页与末页协议.md').read_text(encoding='utf-8'); fund=(ROOT/'11A_本金止盈止损设计与PPT披露协议.md').read_text(encoding='utf-8')
for phrase in ['废止PPT隐藏附录','废止程序化命名和工程痕迹','金额单位统一为“元”','挂机前不预判验证结果','固定第二页已经取消']:
 if phrase not in override: err(f'00A缺少关键规则: {phrase}')
for phrase in ['PRE_RUN_SETUP','POST_RUN_REVIEW','不存在“隐藏附录”这一去向','金额统一使用“元”','固定第二页已经取消']:
...
print(f"VALIDATION_OK version={s.get('当前版本','?')} repo={s.get('仓库','?')} task={task.get('任务ID','?')} ppt_page_types={len(pt)} ppt_tests={len(tests)} ppt=CLEAN_RUNTIME_GATED_YUAN_NO_APPENDIX")
